  ...
  if (file.isFile()) {
    if (file.useCompile && file.ext && file.ext !== '.') {
      ...
      if (file.useCache && cache.revert(revertObj)) {
        ...
        file.setContent(fis.util.read(getReadPath(file)));
        process(file);
        exports.settings.afterCompile(file);
        file.useCache && cache.save(file.getContent(), file.getCacheData());
      }
    } else {
      file.setContent(file.isText() ? fis.util.read(getReadPath(file)) : fis.util.fs.readFileSync(getReadPath(file)));
    }
...
function extHtml(content, callback, file) {
  ...
  //<link>忽略href""中的>,支持类似 <link rel="stylesheet" type="text/css" href="<ctp:out name='fullPath'/>static/common.css">
  var reg = /(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|video|link|object|source)(?:\s+[a-zA-Z]+\s*=\s*(?:'[^']+'|"[^"]+"))*[\s\S]*?(?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)|<ctp:include\s+items="(.*?)"[\s\S]*?(?:>|$)/ig;
  callback = callback || function(m, $1, $2, $3, $4, $5, $6, $7, $8,$9) {
    if ($1) { //<script>
      var embed = '';
      //扩展支持<script>中含有ctp:out,例如<script type="text/javascript" src="<ctp:out name='fullPath'/>static/index.js"></script>
      var reg = /.*<ctp:out.*?>.*/i;
      if(reg.test($1)){
        $1 = $1+$2;
        $2 = null;
      ...
      //m = '<script src="'+$9+'"></script>\r\n';
      //m +=  '<script src="hello.js"></script>';
      m = fis.ctpTag($9).get();
    }
    return m;
  };
      ...
      try {
        switch (type) {
          ...
          case 'jsRequire':
            ...
            var domain = '';
            if (requireConfig.paths[value]) {
              realValue = requireConfig.paths[value];
              domain = requireConfig.domain;
            } else {
              realValue = value;
            }
            info = fis.project.lookup(realValue, file);
            file.addRequire(info.id);

            if (type === 'jsRequire' && info.moduleId) {
              // ret = info.quote + info.moduleId + info.quote;
              // TODO: 这里不应该用info.file的判断来规避问题，应该让他报出来并解决
              ret = info.quote + typeof(info.file) !== 'undefined' ? domain + info.file.getUrl().replace('.js', '') : info.moduleId + info.quote;
            } else {
              ret = info.quote + info.id + info.quote;
            }
          ...
          case 'uri':
            ...
            if (info.file && info.file.isFile()) {
              ...
              if (fis.project.isCtpProject()) {
                if (file.isHtmlLike || ~file.basename.indexOf('.jsp') || ~file.basename.indexOf('.html')) { // html like
                  if (info.jtag) {
                    ret = info.quote + info.jtag + '/hash' + url + query + hash + info.quote;
                  } else {
                    ret = info.quote + info.prefix + 'hash' + url + query + hash + info.quote;
                  }
                } else { // css
                  var parts = info.file.basename.split('.');
                  var name = parts[0];
                  var ext = parts[1];
                  var matches = url.match(name + '_(\\w+)' + '\\.' + ext);
                  if (matches) {
                    var hashed = matches[1];
                    ret = info.quote + info.rest.replace(info.file.basename, name + '_' + hashed + '.' + ext) + query + hash + info.quote;
                  } else {
                    ret = value;  // shall never get here!
                  }
                }
              } else {
...
//20160418,由于每次生成都将发布版的html覆盖原代码版的。所以将原来备份的干净的文件，copy到文件中再进行编译
function getReadPath(file) {
    if (fis.project.isCtpProject() && file.isHtmlLike && fis.util.exists(file.rest + "_bk" + file.rExt)) {
        return file.rest + "_bk" + file.rExt;
    } else {
