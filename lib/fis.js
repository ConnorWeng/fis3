...
function _require(modulename) {
  if (!fis.require.paths.length) {
    var local = fis.get('system.localNPMFolder');
    var global = fis.get('system.globalNPMFolder');

    fis.require.paths = [local];
    if (local !== global) {
      fis.require.paths.push(global);
    }
  }
  var paths = fis.require.paths.filter(function(item, idx, arr) {
  ...
  paths.every(function(dir) {
    try {
      ...
      fis.log.debug('Resolved module %s to %s', modulename, resolved);
    } catch (e) {}
    if (resolved) {
      return false;
    }
