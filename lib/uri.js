...
var uri = module.exports = function(path, dirname) {
  ...
  // Add support for jsp tag
  info.prefix = info.rest.match(/^[\./]*/)[0];
  if (~info.rest.indexOf('<') || ~info.rest.indexOf('${')) {
    var reg = /(\$\{|<)(.+)(>|\})/i;
    var matches = reg.exec(info.rest);
    info.jtag = matches[0];
    // TODO: test every possible path until find the file
    info.rest = info.rest.replace(/(\$\{|<)(.+)(>|\})/ig, '..');
  }
  // end
